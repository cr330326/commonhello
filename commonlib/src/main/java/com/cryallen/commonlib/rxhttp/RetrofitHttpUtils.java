package com.cryallen.commonlib.rxhttp;
...
import com.cryallen.commonlib.rxhttp.download.DownloadHelper;
import com.cryallen.commonlib.rxhttp.factory.ApiFactory;
import com.cryallen.commonlib.rxhttp.manage.RxHttpManager;
import com.cryallen.commonlib.rxhttp.upload.UploadHelper;
...
/***
 * RetrofitCreate 网络请求工具类
 * @author Allen
 * @DATE 2019-09-20
 ***/
public class RetrofitHttpUtils {
	@SuppressLint("StaticFieldLeak")
	private volatile static RetrofitHttpUtils mInstance;
	@SuppressLint("StaticFieldLeak")
	...
	public static RetrofitHttpUtils getInstance() {
		if (mInstance == null) {
			synchronized (RetrofitHttpUtils.class) {
				if (mInstance == null) {
					mInstance = new RetrofitHttpUtils();
				}
			}
		}
		return mInstance;
	 ...
	 */
	public RetrofitHttpUtils init(Context app) {
		mContext = app;
		return this;
	}
	...
	private static void checkInitialize() {
		if (mContext == null) {
			throw new ExceptionInInitializerError("请先在全局Application中调用 RetrofitHttpUtils.getInstance().init(this) 初始化！");
		}
	}
