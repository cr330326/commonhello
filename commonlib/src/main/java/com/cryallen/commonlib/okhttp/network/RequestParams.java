...
import com.cryallen.commonlib.okhttp.body.FileBody;
import com.cryallen.commonlib.okhttp.body.JSONBody;
import com.cryallen.commonlib.okhttp.utils.MediaTypeUtils;
import com.cryallen.commonlib.okhttp.body.OctetStreamBody;
import com.cryallen.commonlib.okhttp.body.ProgressRequestBody;
...
public class RequestParams{
    ...
    public RequestBody requestBody(MediaType mediaType, AbsICallbackHandler callback){
        if (isJSON(mediaType)){
        ...
        //stream
        for (Map.Entry<String, StreamWrapper> streamEntry: streamParams.entrySet()){
            builder.addPart(
        ...
        //file
        for (Map.Entry<String, FileWrapper> file: fileParams.entrySet()){
    ...
    private ProgressRequestBody uploadProgressRequestBody(RequestBody requestBody, AbsICallbackHandler callback){
    ...
    public static class StreamWrapper {
        ...
        static StreamWrapper newInstance(InputStream inputStream, String name, String contentType) {
            return new StreamWrapper(
                    inputStream,
                    name,
                    TextUtils.isEmpty(contentType) ? MediaTypeUtils.APPLICATION_OCTET_STREAM : contentType);
        }
    }
}