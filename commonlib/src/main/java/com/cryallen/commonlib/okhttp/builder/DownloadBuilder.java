...
import com.cryallen.commonlib.okhttp.body.ResponseDownloadBody;
import com.cryallen.commonlib.okhttp.callback.DownloadCallbackHandler;
import com.cryallen.commonlib.okhttp.response.DownloadResponseHandler;
...
public class DownloadBuilder {
    ...
    public DownloadBuilder(OkHttpManager okHttpManager) {
    ...
    public Call enqueue(final DownloadResponseHandler downloadResponseHandler) {
        try {
            ...
            Call call = mOkHttpManager.getOkHttpClient().newBuilder()
                    .addNetworkInterceptor(new Interceptor() {      //设置拦截器
                        @Override
                        public Response intercept(Chain chain) throws IOException {
                            Response originalResponse = chain.proceed(chain.request());
                            return originalResponse.newBuilder()
                                    .body(new ResponseDownloadBody(originalResponse.body(), downloadResponseHandler))
                                    .build();
                        }
                    })
                    .build()
                    .newCall(request);
            call.enqueue(new DownloadCallbackHandler(downloadResponseHandler, mFilePath, mCompleteBytes));
